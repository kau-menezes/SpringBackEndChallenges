...
public class EncodedUserImplementation implements UserService{

    @Autowired
    UserRepository userRepo; 

    @Autowired
    PasswordEncoderService psService;
    ...
    public ResponseEntity<String> createUser(UserData user) {
        ...
        var encodedPassword = psService.encode(user.password());
    ...
    public ResponseEntity<String> changePassword(UserUpdatePassword data) {
        ...
        if (!user.getPassword().matches(data.password()))
        ...
        var newPassword = psService.encode(data.newPassword());
    ...
    public ResponseEntity<String> login(UserData user) {
        ...
        if (!found.getPassword().matches(user.password()))
            return ResponseEntity.status(403).build();
